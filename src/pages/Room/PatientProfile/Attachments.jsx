...
const Attachments = () => {
  ...
  useEffect(() => {
    ...
    // Return a function that unsubscribes from the collection
    return firebase.firestore().collection("patients").onSnapshot(() => {});
    
  }, [ID, firebase.firestore().collection("patients")]);
...
//   console.log("deleted1111", patientID);
//   console.log("deleted2222", imageID);


//   const patientDoc = await patientRef.get();
//   const images = patientDoc.get("images");
//   const index = images.findIndex((image) => image.id === imageID);
//   if (index !== -1) {
//     await patientRef.update({
//       images: firebase.firestore.FieldValue.arrayRemove(images[index]),
//     });
//     console.log("Image deleted successfully!");
//     console.log("deleted", imageID);
//     console.log("deleted1", patientID);
...
const deletePatientData = async (patientID, imageID) => {
  ...
  if (index !== -1) {
    ...
    // Update local image list variable
    const updatedImages = images.filter((image) => image.id !== imageID);

    console.log("Updated patient data:", updatedImages);

    // Display updated image list on UI
    // ...
    patientData()
    deletePatientData(patientID, imageID, patientData);
