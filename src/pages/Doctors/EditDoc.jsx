...
const EditDoc = ({fId,closeEvent}) => {
    ...
    useEffect(()=> {
        console.log("fid",fId.email.id)
        setId(fId.email.id)
        setHospitalId(fId.email.hospitalId)
        setFirstName(fId.email.firstName)
        setLastName(fId.email.lastName)
        setEmail(fId.email.email)
        setRole(fId.email.role)
    ...
    const createUser = async(event,userDt) => {
        ...
        db.collection("users").doc(fId.email.id)
          .update({
